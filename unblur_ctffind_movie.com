...
_rlnDrift #12 """ > starfile_header.txt
...
X_END=`tail -n2 gc_${i%.mrc}_shifts.txt | head -n1 | awk -v frames=50 '{print $frames}'`
X_DIFF=`echo "$X_END - $X_START" | bc -l`
echo "X_DIFF=${X_DIFF}"

Y_START=`tail -n2 gc_${i%.mrc}_shifts.txt | tail -n1 | awk '{print $1}'`
Y_END=`tail -n2 gc_${i%.mrc}_shifts.txt | tail -n1 | awk -v frames=50 '{print $frames}'`
Y_DIFF=`echo "$Y_END - $Y_START" | bc -l`
echo "Y_DIFF=${Y_DIFF}"

DRIFT=`echo "sqrt($X_DIFF^2+$Y_DIFF^2)" | bc -l`
DRIFT=$( printf "%1.1f" $DRIFT )
echo "DRIFT=${DRIFT}"
...
echo "gc_${i%.mrc}_sum_DW.mrc	 gc_${i%.mrc}_ctf.mrc	${def1_A}	${def2_A}	${def_ang}	${AKV}	${CS}	${AC}	${MAG}	${DSTEP}	${res_A}	${DRIFT}" >> gc_${i%.mrc}_ctf.star
...
wait
convert -gravity northwest -pointsize 40 -resize x1024 -fill white -annotate 0x0+0+15 "Def ${def1} µm, ${def2} µm, Res ${res} Å, Drift ${DRIFT} Å" gc_${i%.mrc}_sum.ctf.tif gc_${i%.mrc}_sum.ctf.tif
wait
